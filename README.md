...
Let s create a first package `a`, hosted on `github.com`, as use `mh-cbon`

```sh
mkdir -p $GOPATH/github.com/mh-cbon/a
...
```sh
mkdir -p $GOPATH/github.com/mh-cbon/b
cd $GOPATH/github.com/mh-cbon/b
glide create
touch index.go
...
It is located at `$GOPATH/src`, which makes it loadable from your other go programs/libraries.
...
Let s finish this readme by importing ans consuming a remote dependency. `a` will depend and consume `github.com/Masterminds/semver`
